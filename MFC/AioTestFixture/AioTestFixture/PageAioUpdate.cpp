...
void CPageAioUpdate::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_EDIT_AIO_VERSION, m_EditAIOVersion);
...
BEGIN_MESSAGE_MAP(CPageAioUpdate, CPropertyPage)
    ON_BN_CLICKED(IDC_BTN_GET_AIO_VER, &CPageAioUpdate::OnBnClickedBtnGetAioVer)
    ON_MESSAGE(MSG_AIO_VERSION_UPDATE, &CPageAioUpdate::OnMsgAioVersionUpdate)
    ON_BN_CLICKED(IDC_BTN_AIO_UPDATE, &CPageAioUpdate::OnBnClickedBtnAioUpdate)
END_MESSAGE_MAP()
...
void CPageAioUpdate::OnBnClickedBtnGetAioVer()
{
    BYTE id;

    id = COM_SOFTWARE_VERSION_ID;
    if (g_pSerialProtocol->isSerialOpen())
    {
        m_EditAIOVersion.SetWindowTextA("");
...
int CPageAioUpdate::initApplication(void)
{
    CUpdate::initApplication();
    g_pSerialProtocol->bindPaktFuncByID(COM_SOFTWARE_VERSION_ID ,this, CPageAioUpdate::PktHandleGetVersion);
    return 0;
}


int WINAPI CPageAioUpdate::PktHandleGetVersion(LPVOID pParam, UartProtocolPacket *pPacket)
{
    CPageAioUpdate *pPageUpdate = (CPageAioUpdate *)pParam;
    pPageUpdate->m_EditAIOVersion.SetWindowTextA((char *)&pPacket->DataAndCRC[0]);
    
    HWND hWnd = pPageUpdate->GetSafeHwnd(); 
    if (hWnd != NULL)
    {
...
afx_msg LRESULT CPageAioUpdate::OnMsgAioVersionUpdate(WPARAM wParam, LPARAM lParam)
{
    m_EditAIOVersion.Invalidate();
    return 0;
}


void CPageAioUpdate::OnBnClickedBtnAioUpdate()
{
 ...
 * 返 回 值： BIN文件的个数
 */  
BYTE CPageAioUpdate::detectBinFile(void)
{
...
int CPageAioUpdate::checkTarget(BYTE &CID, CString &path)
{
...
void CPageAioUpdate::displayProgressAIO(BYTE index, int value)
{
    CString str;
    int process = 0;
...
UINT CPageAioUpdate::UpdateThread(LPVOID pParam)
{
    int i;
    unsigned int len;   //bin file lenght
    int result;
    CPageAioUpdate *pSmartUpdate = (CPageAioUpdate *)pParam;
    CString name, str;
    BYTE CID;
    BYTE index = 0;
...
void CPageAioUpdate::addInfo2Display(CString str)
{
...
void CPageAioUpdate::cleanInfo2Display(void)
{
...
BOOL CPageAioUpdate::OnInitDialog()
{
