...
//    char    PipeData[BUF_SIZE]; //add qwb
...
//            CHAR  *ptr = strstr(PipeData, "Press any key to continue . . ."); //qwb
			if ( ptr )
			{
				*ptr = '\0';
			}
			...
			//------------------------------------------------------------------
			AppendText(PipeData);
//			AppendText(CString(PipeData)); //qwb
...
#else //qwb 
            if ( MsgWaitForMultipleObjects( 1, &ProcessInfo.hProcess, FALSE, 0, QS_ALLINPUT) == WAIT_OBJECT_0 )
		    {
			    break;
		    }
#endif
